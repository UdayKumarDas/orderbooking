          ...
          if (url) {
            return $.getJSON(url, function(data) {
              $.each(data, function(index, el) {
                return _this.$targetSelect.append("<option value='" + el.id + "'>" + el.menu_item_name + "</option>");
              });
...
(function() {
  ...
  $('#offer_catId_get').change(function() {
    var category, options;
    category = $('#offer_catId_get :selected').text();
    return options = $(menus).filter("optgroup[lable='" + category + "']").html();
  });
